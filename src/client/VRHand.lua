...
--- VR Hand Base class. 
local VRHand = Class:subclass("VRHand")
---
-- @name VRHand:new()
...
function VRHand:_LoadAnimationTracks()
	local anims = ReplicatedStorage.Content.Animations
	for _, anim in pairs(anims[self.Handedness]:GetChildren()) do
...
function VRHand:ItemGrab()
	...
	local notify_server_grab = Networking.ClientGrab -- tell server what we're up to (picking something up)
